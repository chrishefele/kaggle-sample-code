...
#
# Extended 2/27/2011 to work with spike & nospike files
#

library(glmnet)
library(caTools)

SPIKE_TYPES <- c("spike","nospike")   
for(spike.type in SPIKE_TYPES) {       # repeat whole process for spike & non-spike files 

TRAINING_FILE    <- paste("fordTrain.",spike.type,".csv",sep="")
TEST_FILE        <- paste("fordTest.", spike.type,".csv",sep="")
TEST.PREDICTIONS <- paste("pred.logistic.absdiffs.test.", spike.type,".csv",sep="")
TRAIN.PREDICTIONS<- paste("pred.logistic.absdiffs.train.",spike.type,".csv",sep="")
PLOT_FILE        <- paste("logistic.absdiffs.",spike.type,".plots.pdf",sep="")
...
# Write test & training set probability predictions to a file 
...
write.csv(submission.data, file=TEST.PREDICTIONS, row.names=FALSE,col.names=TRUE)

if(USE_TRAIN_SAMPLES) {
    Prediction <- predict( fit.train, newx=X.train.original, s=LAMBDA, type="response")
} else {
    Prediction <- predict( fit.train, newx=X.train, s=LAMBDA, type="response")
}
TrialID <- train$TrialID
ObsNum <- train$ObsNum
train.pred.data <- data.frame(TrialID, ObsNum, Prediction )
write.csv(train.pred.data, file=TRAIN.PREDICTIONS, row.names=FALSE,col.names=TRUE)
...
} # end of for loop for spike.type 
