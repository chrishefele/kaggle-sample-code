...
SUBMISSION_FILE     <- "logistic.submission.ecdf.t-t.csv"
PLOT_FILE           <- "logistic.plots.ecdf.t-t.pdf"
 
ECDF_MODE           <- "t-t"        #  ecdf used to normalize the data to percentiles before the logistic regression
                                    #  t+t = use distribution from combined test+train data
                                    #  otherwise use ecdf on test/train seperately
...
for(nm in names(X.train)) {
    ...
    if(ECDF_MODE=="t+t") {
        X.train[[nm]]  <- ecdf( trntst.dat )( trn.dat ) 
        X.test[[ nm]]  <- ecdf( trntst.dat )( tst.dat )
    } else {
        X.train[[nm]]  <- ecdf( trn.dat    )( trn.dat ) 
