...
private String  DATA_DIR = "/home/chefele/SemiSupervised/data/data/";

private String  PROBE_FILENAME      = DATA_DIR + "bsvd_probe.csv";
private String  TRAINING_FILENAME   = DATA_DIR + "bsvd_train.csv";
public  String  FILE_FEATURES_TRAIN = DATA_DIR + "bsvd_features_train.csv";
public  String  FILE_FEATURES_TEST  = DATA_DIR + "bsvd_features_test.csv";
...
private int     FEATURES_TRAIN_START =      1; // bsvd feature vec start & end IDs for data output to SVM 
private int     FEATURES_TRAIN_END   =  50000; 
private int     FEATURES_TEST_START  =  50001; 
private int     FEATURES_TEST_END    = 100000; 

private int     MAX_VERTEX_ID = 1100000; // 1000k unlabeled, 50k train, 50k test
private int     PRINT_MOD = 1000000;
...
private int     UNIQUE_VOUTS = 1100000;
private int     UNIQUE_VINS  =  863481; // though the Vin ID range is [1,1M] 
private double  LRATE_VIN_SCALEUP = Math.sqrt(1.0*UNIQUE_VINS/UNIQUE_VOUTS); 
...
public EngineBSVD() { //constructor
    ...
    if (BIDIRECTIONAL_EDGES) {   // directed links, unidirectional 
        ...
        LRATE_VIN_SCALEUP = Math.sqrt(1.0*UNIQUE_VINS/UNIQUE_VOUTS); // con.matrix rectangular
    } 
...
public void calcFeatures(int numFeatures, double regularization,
    ...
    for(int numFeature=0; numFeature<numFeatures; numFeature++) {
        ...
        // for safety, save features after each feature loop (TODO: remove this after code proves in) 
        writeFeatureFile(FILE_FEATURES_TRAIN, fVouts, FEATURES_TRAIN_START, FEATURES_TRAIN_END ); 
        writeFeatureFile(FILE_FEATURES_TEST,  fVouts, FEATURES_TEST_START,  FEATURES_TEST_END  ); 
    }
}
...
public void writeFeatureFile(String featureFileName, 
                             ArrayList<float[]> fVecs,
                             int fValStart, int fValEnd ) {
    System.out.println("Writing features to : "+featureFileName);
    System.out.flush();
    try { 
        BufferedWriter out = new BufferedWriter(new FileWriter(featureFileName)); 
        for(int ix=fValStart; ix<=fValEnd; ix++) {
            String lineOut = ""; 
            float[] lastFVec = fVecs.get(fVecs.size()-1);
            for(float[] fVec : fVecs) {
                if(fVec==lastFVec) {
                    lineOut += fVec[ix]+"\n"; 
                } else {
                    lineOut += fVec[ix]+","; 
        ...
        System.out.println("ERROR writing to    : "+featureFileName); 
    } 
}
