...
PLOT_FILE       <- 'logistic-pcaimagepix.pdf'
SUBMISSION_FILE <- 'logistic-pcaimagepix.csv'
...
rot_whale     <- as.matrix(prcomp(as.matrix(train[labels==1]))$rotation)
rot_nowhale   <- as.matrix(prcomp(as.matrix(train[labels==0]))$rotation)
train_whale   <- as.matrix(train) %*% rot_whale
train_nowhale <- as.matrix(train) %*% rot_nowhale
