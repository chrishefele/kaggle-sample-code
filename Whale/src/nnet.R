...
PLOT_FILE       <- 'nnet.pdf'
SUBMISSION_FILE <- 'nnet.csv'

NHIDDEN <- 10 # hidden neurons in middle layter
...
system.time( 
    fit.train <- nnet(X.train, Y.train, size=NHIDDEN, entropy=TRUE)
)
...
Y.train.predictions <- predict(fit.train, newdata=X.train, type="raw")
print("AUC")
