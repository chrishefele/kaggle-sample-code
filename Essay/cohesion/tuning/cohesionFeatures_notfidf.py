...
class LSA_Engine:
    ...
    def trainLSA(self, essay_train, num_topics):
        ...
        self.model_lsi  = gensim.models.lsimodel.LsiModel(corpus, id2word=self.dictionary, num_topics=num_topics)
    ...
    def scoreEssay(self, essay, **kwargs):
        ...
        essay_corpus_mapped = self.model_lsi[ essay_corpus ]
        ...
        assert nrow == ncol
        if nrow > 1:
            sim_adjacent        = numpy.mean(  sent_sims_matrix.diagonal(1)) # off-diagonal
            sim_adjacent_median = numpy.median(sent_sims_matrix.diagonal(1)) # off-diagonal
            sim_all = (sent_sims_matrix.sum() - sent_sims_matrix.diagonal().sum()) \
                      / float(nrow*ncol - nrow)
            sim_ratio = sim_adjacent / sim_all
        else:
            sim_adjacent, sim_adjacent_median, sim_all, sim_ratio  = 0.0, 0.0, 0.0, 1.0
        v = {}
        suffix = '_%04i' % self.num_topics
        v['CH_cohesionAdjacent'+suffix]          = sim_adjacent
        v['CH_cohesionAdjacentMedian'+suffix]    = sim_adjacent_median
        v['CH_cohesionAll'+suffix]         = sim_all
        v['CH_cohesionAdjAllRatio'+suffix] = sim_ratio
        return v
...
def printEssayStats(essay_data):
    ...
    for essay_set in word_counts:
        ...
        print "Words:", len(word_counts[essay_set]), 
...
def writeFeatures(num_topics, ftrain, ftest, fout):
    ...
    print "LDA training   file:", ftrain
