...
class LSA_Engine:
    ...
    def topicEdgeFeatures(self, sims_matrix_raw):
        nrow, ncol = sims_matrix_raw.shape
        assert nrow>0 and ncol>0
        if nrow > 1: 
            sims_raw = sims_matrix_raw.diagonal(1)
        else:
            sims_raw = numpy.array([0])
        sims_delta = sims_raw - numpy.median(sims_raw)  
        edges_binary = sum(numpy.abs(numpy.ediff1d(1*(sims_delta>0), to_end=0)))
        edges_analog = sum(numpy.abs(numpy.ediff1d(1*(sims_delta  ), to_end=0)))
        v = {}
        v['CH_cohesionTopicEdgeNum'] = edges_binary 
        v['CH_cohesionTopicEdgeSum'] = edges_analog
        return v
    ...
    def scoreEssay(self, essay, **kwargs):
        ...
        # Now add on some new features
        for feature_name, feature_value in self.topicEdgeFeatures(sent_sims_matrix).items():
            v[feature_name+suffix] = feature_value
