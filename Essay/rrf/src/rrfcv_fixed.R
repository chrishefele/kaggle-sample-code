    ...
    ... ) 
{
    ...
    if (classRF) {
        ...
        # f <- cut(trainy, c(-Inf, quantile(trainy, 1:4/5), Inf))    
          f <- cut(trainy, unique(trainy) )  # *** FIX 
    }
    nlvl <- table(f)
    idx <- numeric(n)
    ...
    for (i in 1:cv.fold) {
        all.rf <- RRF(trainx[idx != i, , drop = FALSE], trainy[idx != 
            i], trainx[idx == i, , drop = FALSE], trainy[idx == 
            i], mtry = mtry(p), importance = TRUE, ...)
        cv.pred[[1]][idx == i] <- all.rf$test$predicted
        impvar <- (1:p)[order(all.rf$importance[, 1], decreasing = TRUE)]
        for (j in 2:k) {
            imp.idx <- impvar[1:n.var[j]]
            sub.rf <- RRF(trainx[idx != i, imp.idx, drop = FALSE], 
                trainy[idx != i], trainx[idx == i, imp.idx, drop = FALSE], 
