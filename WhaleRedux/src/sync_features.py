# sync_features.py
#
# Usage:  python sync_features.py   feature_file1.csv  feature_file2.csv ...
#
# This program rewrites feature files so they all have a common set of 
# columns/features. Unique columns may appear in a file if one value of a 
# variable appears in only one dataset. 
# Any columns/features unique to one file are deleted.
...
for fname in fnames:
    ...
    if REWRITE:
        ...
        for uniq_name in uniq_names:
            del df[uniq_name]
        df.set_index('clip', inplace=True)
