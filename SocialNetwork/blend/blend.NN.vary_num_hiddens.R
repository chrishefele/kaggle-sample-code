...
for(nHiddens in 1:64) {

    system.time( fit.probe <- nnet(X.probe, Y.probe, size=nHiddens) )
    print(fit.probe)
    summary(fit.probe)

    # now write test set predictions to a file for submission
    #Y.test <- predict(fit.probe, X.test)
    #submission.data <- data.frame(test$Vout, test$Vin, Y.test )
    #write.csv(submission.data,file="blend.out.NN.test.csv",row.names=FALSE,col.names=TRUE)

    # check AUC of cv-fit against known probe values 
    Y.probe.pred   <- predict(fit.probe, X.probe )
