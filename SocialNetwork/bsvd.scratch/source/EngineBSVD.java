...
//private String  PROBE_FILENAME    = DATA_DIR + "social_probe_sample10K.csv";
...
private float   SELF_LINK_EDGE_PROB = 0.0f;  // link to self edge probability 
...
public void addSelfLinks(ArrayList<EdgeRecord> edgeRecords) {
    for(int Vid=0; Vid<=MAX_VERTEX_ID; Vid++) {
        if ((VoutIDs.contains(Vid)) || (VinIDs.contains(Vid))) {
            addEdge(Vid, Vid, SELF_LINK_EDGE_PROB, edgeRecords); 
        }
    }
}
...
public double predictEdgeProb(int Vout, int Vin, int numFeatures) {
    double sumProds = 0.0; 
    for(int numFeature=0; numFeature < numFeatures; numFeature++) {
...
public double predictEdgeProb(int Vout, int Vin) {
    ...
    return predictEdgeProb(Vout, Vin, numFeatures);
}
...
public void writeEdgeFileAllFPreds(String edgeFileName, ArrayList<EdgeRecord> edgeRecords) {
    try { 
        BufferedWriter out = new BufferedWriter(new FileWriter(edgeFileName)); 
        for(EdgeRecord edge : edgeRecords) {
            String lineOut = edge.Vout+","+edge.Vin;
            int maxNumFeatures = fVouts.size();
            for(int nFeatures=0; nFeatures <= maxNumFeatures; nFeatures++) {
                double edgeProbPrediction = predictEdgeProb(edge.Vout,edge.Vin,nFeatures);
                lineOut += (","+edgeProbPrediction);
            }
            lineOut += "\n";
            out.write(lineOut);
        }
...
public void go( int     numFeatures,    // excludes bias
    ...
    addSelfLinks(trainEdgeRecords); // self links improve probeLL by ~0.0015 
    ...
    writeEdgeFileAllFPreds("allFPreds."+PREDICTIONS_PROBE, probeEdgeRecords); 
