...
# Experimental training sets; at most 1 of these 2 flags should be true
# If false/false, defaults to "regular" regression 
# probe = last 50k lines as a holdout set
TRAIN_ON_PROBE_ONLY             <- FALSE # Train on probe only, exclude training. 
TRAIN_ON_TRAINING_PLUS_PROBE    <- FALSE # Train on probe plus training set (original training set).
# Note: both of above make probe predictions & RMSE overfit; testing predictions are OK

TRAINING_SKIP_OPEN <- TRUE  # train on the first few minutes of the trading day?
OPEN_DURATION      <- 120   # market open duration, in seconds
                            # Note: 10% of days' sqerr in first 1 minute, 25% in first 5 minutes
...
PLOT_FILE       <- "regression.plots.pdf"
SUBMISSION_FILE <- "regression.testing.csv"
PROBE_FILE      <- "regression.probe.csv"
...
if(TRAIN_ON_PROBE_ONLY) {    
    training <- probe
    training.features <- probe.features
}

if(TRAIN_ON_TRAINING_PLUS_PROBE) { 
    training <- rbind(training, probe)
    training.features <- rbind(training.features, probe.features)
}
...
makeX   <- function(init, prices, features) {   
        ...
        as.matrix(cbind(  prices[init.mask,ask.names.pre] - askRef(init, prices),       
                          prices[init.mask,bid.names.pre] - bidRef(init, prices), 
                          features[init.mask,]  
        ))   
}
...
price.sd <- function(df, names.pre) {
        sid <- df$security_id
        prices.pre  <- as.matrix(cbind(df[,names.pre]))
...
makeXwts <- function(init, training.data, target.data, features) {  # makes per-security row weights for training 
        ...
        ratio.spread50s  <- tapply( target.data$ask50   - target.data$bid50,   target.data$security_id,   mean) /
                            tapply( training.data$ask50 - training.data$bid50, training.data$security_id, mean)
...
makeY <- function(init, prices, price.names, price.offset) { 
        print("Making Y for:")
        print(price.names)
...
myPredict  <- function(fit.model, new.X, price.offset) { 
...
# skip training on volitile market open, which throws off the regression
if(TRAINING_SKIP_OPEN) {
    print("NOT training on market open")
    t.events  <- parseHMS(training$time49)
    t.mask    <- as.numeric(t.events - min(t.events)) > OPEN_DURATION 
    training            <- training[         t.mask, ]
    training.features   <- training.features[t.mask, ]
    gc()
} else {
    print("Training on market open")
}
...
for(i in 1:nitems ) {
    ...
    for(initiator in INITIATORS) {
        ...
        Xwts      <- makeXwts(initiator, training, probe, training.features)   # *** TUNABLE!!! probe or testing
        ...
        Y.asks    <- makeY(initiator, training, target.ask.name.post, askRef(initiator,training) ) 
        Y.bids    <- makeY(initiator, training, target.bid.name.post, bidRef(initiator,training) ) 
        ...
        preds.probe[imask, target.ask.name.post] <- myPredict(fit.asks, X, askRef(initiator,probe))
        preds.probe[imask, target.bid.name.post] <- myPredict(fit.bids, X, bidRef(initiator,probe))
        ...
        preds.testing[imask, target.ask.name.post] <- myPredict(fit.asks, X, askRef(initiator,testing))
        preds.testing[imask, target.bid.name.post] <- myPredict(fit.bids, X, bidRef(initiator,testing))
