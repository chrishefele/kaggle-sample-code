...
PLOTFILE <- "plot_avgtseries_pca_t1-t100.pdf"
...
plotPrCompVsTime <- function(in.dat, in.tag) { 
    N_PCS <- 3 # number of principal components to plot
    asks1 <- cbind(in.dat[all.asks])  # 1 row/event, cols are time periods
    bids1 <- cbind(in.dat[all.bids])  
